...
export default function LikedUsers({ navigation }: any) {
  const route = useRoute();
  const { storyId } = route.params as { storyId: number };
  const [users, setFollowerList] = useState<User[]>([]);
  useEffect(() => {
    const fetchUser = async () => {
      const url = `${API_URL}/users/Likelist/${storyId}`;
        ...
        {users?.map((user) => (
          ...
          >
            ...
            <LottieView
              ...
              loop
            />
            <Text style={{ marginTop: 20 }}>No one liked this story!</Text>
          </View> }
      </>
